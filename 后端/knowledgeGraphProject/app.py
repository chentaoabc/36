...
from creat import get_newdata,creat_node, inference, threating_test
...
@app.route('/')
def hello_world():
    t = threading.Thread(target=threating_test)
    t.run()
    return 'Hello World!'
...
def add_node():
    ...
    ###开启推理的一个进程
    time.sleep(5)
    t = threading.Thread(target=inference, args=(setData,))
